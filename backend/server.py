...
app = FastAPI(
    title="HR FAQ API", version="2.0.0", description="HR FAQ Chatbot with RAG support"
)
...
def load_rag_engine():
    ...
    try:
        ...
        rag_engine = get_rag_engine(
            company_data_path="company_data/techcorp_solutions", force_init=True
        )
...
def get_fallback_response(
    question: str, mode: str, context: str = ""
) -> tuple[str, bool]:
...
async def ask_stream(request: AskRequest):
    ...
    async def generate_stream() -> AsyncGenerator[str, None]:
        ...
        if not is_hr_related(question):
            ...
            if rag_engine and rag_engine.is_initialized:
                company_name = rag_engine.get_company_name()
                context, raw_sources = rag_engine.get_context_for_question(
                    question, mode
                )
...
async def rebuild_index():
    ...
    try:
        ...
        if rag_engine.is_initialized:
            ...
            return {
                "status": "partial",
                "message": "RAG engine initialized but dependencies may be missing",
            }
    except Exception as e:
        raise HTTPException(
