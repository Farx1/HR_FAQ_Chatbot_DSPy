...
def evaluate_model():
    ...
    print(f"\nCriteria Check:")
    print(f"  HR Relevance (EM >= 0.3): {'PASS' if avg_em >= 0.3 else 'FAIL'} ({avg_em:.3f})")
    print(f"  OOD Rejection (>= 0.5): {'PASS' if avg_ood_rejection >= 0.5 else 'FAIL'} ({avg_ood_rejection:.3f})")
    ...
    results = {
        "summary": {
            ...
            "avg_ood_rejection": float(avg_ood_rejection),
            "criteria_met": bool(avg_em >= 0.3 and avg_ood_rejection >= 0.5)
        },
