...
class HRFAQAdapter(dspy.BaseLM):
    ...
    def _load_model(self):
        ...
        try:
            ...
            print(
                f"Warning: Could not load tokenizer from {self.model_path}, using base model tokenizer: {e}"
            )
        ...
        try:
            self.pytorch_model = PeftModel.from_pretrained(
            ...
            if os.path.exists(config_path):
                ...
                adapter_weights_path = os.path.join(
                    self.adapter_path, "adapter_model.bin"
                )
                ...
                if os.path.exists(adapter_weights_path):
                    ...
                    try:
                        state_dict = torch.load(
                            adapter_weights_path, map_location="cpu"
                        )
...
class HRFAQSignature(dspy.Signature):
    ...
    question: str = dspy.InputField(desc="The HR-related question from the employee")
...
class HRFAQRejectionSignature(dspy.Signature):
    ...
    question: str = dspy.InputField(desc="The question to evaluate")
...
class HRFAQWithRejection(dspy.Module):
    ...
    def forward(self, question: str):
        ...
        return dspy.Prediction(
            answer=result.answer, is_hr_related=True, reasoning=classification.reasoning
        )
