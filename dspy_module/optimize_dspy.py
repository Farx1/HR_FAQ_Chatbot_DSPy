...
def optimize_module():
    ...
    if valset:
        ...
        evaluate = Evaluate(
            devset=valset, metric=metric, num_threads=1, display_progress=True
        )
    ...
    try:
        ...
        optimizer = BootstrapFewShot(
            metric=metric, max_bootstrapped_demos=4, max_labeled_demos=2, max_rounds=1
        )

        optimized_module = optimizer.compile(
            module, trainset=trainset, valset=valset if valset else trainset
        )
