...
def load_baseline_model():
    ...
    try:
        ...
        if os.path.exists(config_path):
            ...
            adapter_weights_path = "models/hr_faq_dialogpt_lora_adapters/adapter_model.bin"
            if not os.path.exists(adapter_weights_path):
                adapter_weights_path = "models/hr_faq_dialogpt_lora_adapters/adapter_model.safetensors"
...
def evaluate_dspy(hr_data: List[Dict], ood_data: List[Dict], optimized: bool = False):
    ...
    for example in ood_data:
        ...
        is_rejection = (
            any(keyword in generated_lower for keyword in rejection_keywords) or
            "outside the scope" in generated_lower or
            "outside of hr" in generated_lower or
            "non-hr" in generated_lower or
            ("contact" in generated_lower and "hr" in generated_lower and len(generated) > 50)
        )
...
def compare_results(baseline_results: Dict, dspy_results: Dict):
    ...
    print(f"OOD Improvement: {dspy_summary['avg_ood_rejection'] - baseline_summary['avg_ood_rejection']:+.3f}")
...
def main():
    ...
    comparison_results = {
        ...
        "dspy": dspy_results,
        "comparison": {
            "hr_em_diff": dspy_results["summary"]["avg_exact_match"] - baseline_results["summary"]["avg_exact_match"],
            "hr_rouge_diff": dspy_results["summary"]["avg_rouge_l"] - baseline_results["summary"]["avg_rouge_l"],
            "hr_bleu_diff": dspy_results["summary"]["avg_bleu"] - baseline_results["summary"]["avg_bleu"],
            "ood_rejection_diff": dspy_results["summary"]["avg_ood_rejection"] - baseline_results["summary"]["avg_ood_rejection"]
        }
    }
